...
namespace dxvk::hud {
  ...
  public:
    ...
    VkExtent2D surfaceSize() const {
      Logger::warn("Surface sizes");
      VkExtent2D extent = VkExtent2D { m_dstView->imageInfo().extent.width, m_dstView->imageInfo().extent.height };
      Logger::warn("done Surface sizes");
      return extent;
    }
