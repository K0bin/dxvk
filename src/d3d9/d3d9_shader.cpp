...
namespace dxvk {
  ...
  public:
    ...
    void convertShader(
      ...
      options.isSWVP = m_options.isSWVP;
  ...
  D3D9CommonShader::D3D9CommonShader(
    ...
    if (dumpPath.size() != 0) {
      ...
      file.write(reinterpret_cast<const char*>(pShaderBytecode), ModuleInfo.bytecodeLength);
    }

    D3D9ShaderOptions options = { };
    options.isSWVP = Prepass.getShaderInfo().getType() == dxbc_spv::sm3::ShaderType::eVertex && pDevice->CanSWVP();
    options.d3d9FloatEmulation = pDevice->GetOptions()->d3d9FloatEmulation;
    options.forceSamplerTypeSpecConstants = pDevice->GetOptions()->forceSamplerTypeSpecConstants;
    ...
    if (true) {
      ...
      CreateIrShader(pDevice, ShaderKey, createInfo, options, pShaderBytecode, Prepass.getLength());
    } else {
    ...
    m_constants            = Prepass.getConstantsInfo();
    m_immediateConstants   = Prepass.getImmediateConstants();
  ...
  void D3D9CommonShader::CreateLegacyShader(
    ...
    analysisInfo.bytecodeByteLength = ModuleInfo.bytecodeLength;
    // TODO: coissue

    m_shader = module.compile(moduleInfo, ShaderKey.toString(), analysisInfo, constantLayout);
  }
  ...
  HRESULT D3D9ShaderModuleSet::GetShaderModule(
    ...
    *pShader = D3D9CommonShader(pDevice, ShaderKey, Prepass, ModuleInfo, pShaderBytecode);
