...
namespace dxvk {
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::UpdateTexture(
    ...
    for (uint32_t a = 0; a < arraySlices; a++) {
      ...
      for (uint32_t m = 0; m < mipLevels; m++) {
        uint32_t subresource = dstTexInfo->CalcSubresource(a, m);
        ...
        UpdateTextureFromBuffer(dstTexInfo, srcTexInfo, subresource, subresource, offset, extent, offset);
        dstTexInfo->SetWrittenByGPU(subresource, true);
      }
    }
  ...
  HRESULT D3D9DeviceEx::FlushImage(
    ...
    VkOffset3D mip0Offset = { int32_t(box.Left), int32_t(box.Top), int32_t(box.Front) };
    VkOffset3D offset = {
      mip0Offset.x >> subresource.mipLevel,
      mip0Offset.y >> subresource.mipLevel,
      mip0Offset.z >> subresource.mipLevel
    };
  ...
  void D3D9DeviceEx::UpdateTextureFromBuffer(
    ...
    auto formatInfo  = imageFormatInfo(image->info().format);
    auto subresource = pDestTexture->GetSubresourceFromIndex(
    ...
    if (likely(convertFormat.FormatType == D3D9ConversionFormat_None)) {
      ...
      alignedExtent = util::snapExtent3D(alignedDestOffset, alignedExtent, texLevelExtent);
      alignedExtent = util::snapExtent3D(alignedSrcOffset, alignedExtent, texLevelExtent);
      ...
      VkDeviceSize copySrcOffset = (srcOffsetBlockCount.z * texLevelExtentBlockCount.height * texLevelExtentBlockCount.width
          + srcOffsetBlockCount.y * texLevelExtentBlockCount.width
          + srcOffsetBlockCount.x)
