...
namespace dxvk {
  ...
  D3D9DeviceEx::D3D9DeviceEx(
    ...
    if (m_d3d9Options.ffUbershaderVS) {
      BindFFUbershader<DxsoProgramType::VertexShader>();
    }
    if (m_d3d9Options.ffUbershaderFS) {
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::SetVertexDeclaration(IDirect3DVertexDeclaration9* pDecl) {
    ...
    const bool wasUsingFFUbershader = m_d3d9Options.ffUbershaderVS && !UseProgrammableVS();

    m_state.vertexDecl = decl;

    const bool usesFFUbershader = m_d3d9Options.ffUbershaderVS && !UseProgrammableVS();
    if (unlikely(usesFFUbershader && !wasUsingFFUbershader)) {
      BindFFUbershader<DxsoProgramType::VertexShader>();
    }
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::SetVertexShader(IDirect3DVertexShader9* pShader) {
    ...
    } else if (m_d3d9Options.ffUbershaderVS) {
      BindFFUbershader<DxsoProgramType::VertexShader>();
    }
