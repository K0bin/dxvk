...
namespace dxvk {
  ...
  void D3D9FFShaderCompiler::compileVS() {
    ...
    for (uint32_t i = 0; i < caps::TextureStageCount; i++) {
      ...
      uint32_t projectedBit = m_spec.get(m_module, m_specUbo, SpecProjected, i, 1);
  ...
  void D3D9FFShaderCompiler::compilePS() {
    ...
    for (uint32_t i = 0; i < caps::TextureStageCount; i++) {
      ...
      auto DoProjection = [&](uint32_t texcoordId, uint32_t samplerIndex) {
        ...
        uint32_t shouldProj = m_spec.get(m_module, m_specUbo, SpecProjected, samplerIndex, 1);
      ...
      auto SampleType = [this, &SampleImage, unboundTextureConstId](uint32_t textureStage, D3D9FFSamplerType samplerType, uint32_t texcoordId, uint32_t previousTextureValId) {
        ...
        if (samplerType != SamplerTypeTexture3D) {
          ...
          uint32_t isDepth = m_spec.get(m_module, m_specUbo, SpecSamplerIsDepth, textureStage, 1);
        ...
        uint32_t isNull = m_spec.get(m_module, m_specUbo, SpecSamplerIsNull, textureStage, 1);
  ...
  D3D9FFShader::D3D9FFShader(
    ...
    if (isVS) {
      ...
      info.bindingCount = bindings.size();
      info.bindings = bindings.data();
      info.inputMask = (1u << 11u) - 1u; // All inputs 0 - 11 (incl.)
      info.outputMask = 1u; // Output 0
